...
from zigbee_control import call_service # Custom module for Zigbee control (e.g., to interact with smart devices)
...
if args.Live:
   ...
   x1,y1,w1,h1 = 900,100,700,700
   person_detect_start = False
   person_detect_start1 = False
   ...
   while True:
       ...
       image1, detection1 = process_frame_live(roi1, input_width, input_height, interpreter,
                                  input_details, output_details, labels,
                                  boxes_idx, classes_idx, scores_idx, args.threshold)     
       ...
       if len(detection):
           ...
           if (time.time() - person_detect_start_time > duration_threshold):
                ...
                print("detected person image saved as:",file_name_detect)
                # Call a Zigbee service to turn on a switch (e.g., a light).
                switch  = "switch.sonoff_s26r2zb"
                # toggle switch
                call_service("switch", "toggle", {"entity_id": switch})
                print(f"Requested {switch} to toggle")
                # Wait for a second before continuing to avoid multiple detections.
                time.sleep(2)
       elif (len(detection1)):
            print("in the detection zone 2")
            if not(person_detect_start1):
               person_detect_start1 = True
               person_detect_start_time1 = time.time()
               print("start time for person:",person_detect_start_time1)
          
            if (time.time() - person_detect_start_time1 > duration_threshold):
                cv2.putText(frame, "Person Detected in Zone2", (900, 60), cv2.FONT_HERSHEY_SIMPLEX,
                   2, (10, 30, 100), 5, cv2.LINE_AA)
                print("person here in the room")
                file_name_detect = 'detect.jpg'
                detected_person = cv2.imwrite(file_name_detect, image)
