...
def send_discord_audio(file_path: str, message: str = ""):
    url = config['discord_webhook_url']
    with open(file_path, 'rb') as f:
        files = {
    ...
    if response.status_code in (200, 204):
        print("✅ Discord: Audio sent successfully!")
    else:
        print(f"❌ Discord: Failed to send audio ({response.status_code}) ->", response.text)
...
def send_telegram_audio(file_path: str, caption: str = None):
    ...
    if response.status_code == 200:
        ...
        print("❌ Telegram: Failed to send audio:", response.text)
