...
def draw_detections_live(image, boxes, classes, scores, labels, threshold=0.5):
   # -------------------------------------------------------
   # Draw bounding boxes and labels on the image
   # -------------------------------------------------------
   detections = []
   for i in range(len(scores)):
       if scores[i] >= threshold:
           ...
           cv2.rectangle(image, (xmin, ymin), (xmax, ymax), (200, 40, 0), 5)
           cv2.putText(image, f'{label}: {score:.2f}', (xmin, ymin - 5),
                       cv2.FONT_HERSHEY_SIMPLEX, 1, (200, 50, 200), 5)
...
# Function to Run Inference on Frame
# ----------------------------
def process_frame_live(frame,input_width, input_height, interpreter, input_details, output_details, labels, boxes_idx, classes_idx, scores_idx, threshold=0.5):
   # -------------------------------------------------------
   # Preprocess the frame for model input
   # -------------------------------------------------------
   # Resize the frame to the input size expected by the model
   ...
   detection = draw_detections_live(frame, boxes, classes, scores, labels, threshold=threshold)
